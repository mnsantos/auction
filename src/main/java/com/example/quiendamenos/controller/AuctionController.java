...
public class AuctionController {
    ...
    public ResponseEntity<PremiumResponse> premium(@RequestParam int id, @RequestParam int type) {
        ...
        switch (type) {
            case (1):
                premiumResponse = auctionService.bestOccupied(id, 1.0);
                break;
            case (2):
                premiumResponse = auctionService.bestOccupied(id, 0.1);
                break;
            case (3):
                premiumResponse = auctionService.bestPlaceOrOccupied(id, 1.0);
                break;
            case (4):
                premiumResponse = auctionService.bestPlaceOrOccupied(id, 0.1);
                break;
            default:
